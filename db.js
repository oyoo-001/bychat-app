...
// MODIFIED: This function is now more robust against bad input.
async function getLatestMessages(limit = 100) {
    try {
        // Ensure 'limit' is a valid number before it's passed to the query.
        // This prevents errors if the function is called with null, undefined, or a non-numeric string.
...
    try {
        const queryLimit = Number(limit) || 50; // Also made this more robust
        const [rows] = await db.execute( // Using 'db' for consistency
